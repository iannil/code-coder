...
export namespace Permission {
  ...
  // Permission handler callback support
  export type AskCallback = (info: Info) => Promise<"once" | "always" | "reject">
  let handlerCallback: AskCallback | undefined

  export function setHandler(callback: AskCallback) {
  ...
  export async function ask(input: {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      // If callback handler is set, use it instead of waiting for event bus response
      if (handlerCallback) {
        handlerCallback(info)
          .then((response) => {
            respond({
              sessionID: info.sessionID,
          ...
          .catch((error) => {
            log.error("permission handler error", { error })
            respond({
              sessionID: info.sessionID,
              permissionID: info.id,
              response: "reject",
