...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    const [store, setStore] = createStore({
      ...
      active: (sync.data.config.theme ?? kv.get("theme", "tokyonight")) as string,
      ready: false,
    })
    ...
    function resolveSystemTheme() {
      ...
      renderer
        ...
        .then((colors) => {
          Log.Default.debug("colors.palette", colors.palette)
          if (!colors.palette[0]) {
            if (store.active === "system") {
              setStore(
                produce((draft) => {
                  draft.active = "tokyonight"
                  draft.ready = true
                }),
              )
            }
    ...
    const values = createMemo(() => {
      return resolveTheme(store.themes[store.active] ?? store.themes.tokyonight, store.mode)
    })
