...
export function PermissionPrompt(props: { request: PermissionRequest }) {
  ...
  const input = createMemo(() => {
    if (!props.request.callID) return {}
    const parts = sync.data.part[props.request.messageID] ?? []
    for (const part of parts) {
      if (part.type === "tool" && part.callID === props.request.callID && part.state.status !== "pending") {
          ...
          body={
            <Switch>
              <Match when={props.request.always.length === 1 && props.request.always[0] === "*"}>
                <TextBody title={"This will allow " + props.request.permission + " until CodeCoder is restarted."} />
              </Match>
              <Match when={true}>
                <box paddingLeft={1} gap={1}>
                  <text fg={theme.textMuted}>This will allow the following patterns until CodeCoder is restarted</text>
                  <box>
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell CodeCoder what to do differently</text>
        </box>
      </box>
      <box
