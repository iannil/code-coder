...
export function QuestionPrompt(props: { request: QuestionRequest }) {
  ...
  function submit() {
    ...
    const answers = questions().map((_: any, i: number) => store.answers[i] ?? [])
  ...
  useKeyboard((evt) => {
    // Skip processing if already completed or a dialog (e.g., command palette) is open
    if (store.completed || dialog.stack.length > 0) return
    ...
    if (store.editing && !confirm()) {
      ...
      if (evt.name === "return") {
        ...
        if (!text) {
          if (prev) {
            ...
            const answers = [...store.answers]
            answers[store.tab] = (answers[store.tab] ?? []).filter((x: string) => x !== prev)
            setStore("answers", answers)
          }
          setStore("editing", false)
          return
      ...
      // Don't return here - let other event handlers process the event
      // This allows textarea to receive keyboard input
    }
    ...
    // Skip remaining key handling when editing
    if (store.editing && !confirm()) {
      return
    }
