...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, codecoderModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.codecoder?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.codecoder?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          codecoder: { itemId: "msg_codecoder" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              codecoder: { itemId: "msg_codecoder_part" },
    ...
    expect(result[0].providerOptions?.codecoder?.itemId).toBe("msg_codecoder")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.codecoder?.itemId).toBe("msg_codecoder_part")
