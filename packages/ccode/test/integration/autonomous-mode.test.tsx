...
import { RollbackManager, createRollbackManager, type RollbackTrigger } from "@/autonomous/safety/rollback"
import {
  SafetyIntegration,
...
describe.skip("Crazy Mode Integration", () => {
  ...
  describe("StateMachine", () => {
    ...
    it("should transition to PLANNING state", async () => {
      await stateMachine.transition(AutonomousState.PLANNING, { reason: "Starting to plan" })
    ...
    it("should track state history", async () => {
      ...
      await stateMachine.transition(AutonomousState.EXECUTING, { reason: "Second" })
      await stateMachine.transition(AutonomousState.TESTING, { reason: "Third" })
  ...
  describe("SafetyGuardrails", () => {
    it("should detect state oscillation loops", () => {
      guardrails.recordStateTransition(AutonomousState.PLANNING, AutonomousState.EXECUTING)
      guardrails.recordStateTransition(AutonomousState.EXECUTING, AutonomousState.PLANNING)
      guardrails.recordStateTransition(AutonomousState.PLANNING, AutonomousState.EXECUTING)
  ...
  describe("RollbackManager", () => {
    ...
    test("should track rollback count and retry limit", async () => {
  ...
  describe("Orchestrator Integration", () => {
    it("should create and initialize orchestrator with safety integration", async () => {
      ...
      const orchestrator = createOrchestrator({ sessionId, requestId: "test_req", request: "Test request" }, config)
    ...
    it("should handle blocked decisions correctly", async () => {
      ...
      const orchestrator = createOrchestrator({ sessionId, requestId: "test_req", request: "Test" }, config)
  ...
  describe("AutonomousEvent System", () => {
    it("should define all required events", () => {
      ...
      for (const event of events) {
        expect(AutonomousEvent[event]).toBeDefined()
        expect(typeof AutonomousEvent[event]).toBe("function")
        expect(typeof AutonomousEvent[event]?.define).toBe("function")
      }
    })

    it("should have correct event schemas", () => {
      // Verify ResourceWarning has destructive_operation type
      const resourceWarningSchema = AutonomousEvent.ResourceWarning.schema
