...
fn render_help_panel(frame: &mut Frame, area: Rect, theme: &crate::tui::Theme) {
    ...
    let help_lines = vec![
        ...
        row("Ctrl+Z",       "Undo input", primary),
        row("Ctrl+Shift+Z", "Redo input", primary),
        row("Ctrl+A / E",   "Cursor to line start / end", primary),
        row("Ctrl+W",       "Kill word backward (→ kill-ring)", primary),
        row("Ctrl+U / K",   "Kill to start / end of line (→ kill-ring)", primary),
        row("Ctrl+Y",       "Yank (paste kill-ring)", primary),
