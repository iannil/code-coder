...
import { $ } from "bun"
import { createCodecoder } from "@codecoder-ai/sdk"
...
export async function getLatestRelease() {
  return fetch("https://api.github.com/repos/iannil/code-coder/releases/latest")
    .then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/iannil/code-coder/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log = await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/ccode`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/ccode/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/ccode/")) areas.add("core")
    }
...
async function summarizeCommit(
  codecoder: Awaited<ReturnType<typeof createCodecoder>>,
  message: string,
): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await codecoder.client.session.create()
  const result = await codecoder.client.session
    .prompt({
      path: { id: session.data!.id },
      body: {
        model: { providerID: "codecoder", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], codecoder: Awaited<ReturnType<typeof createCodecoder>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(codecoder, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/iannil/code-coder/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const codecoder = await createCodecoder({ port: 5044 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, codecoder)
    notes.push(...lines)
