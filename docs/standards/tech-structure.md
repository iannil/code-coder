...
### Phase 0: 商业闭环与项目立项 (商务/管理阶段) ☑️

**目标：** 明确边界，算清投入产出，完成内部 OA 流程，拿到开发资源（时间、服务器、API预算）。
**预计耗时：** 1-2 周

* [x] **架构设计 (Architecture V2)**：在现有架构上补充”逆向引擎”、”移动端Agent”和”资产调度器”。☑️ *五层一环架构已落地，Rust 微服务拆分完成*
* [ ] **需求分析 (PRD)**：
...
### Phase 1: 核心基座与”对话即执行” (MVP 最小可行性产品) ☑️

**目标：** 把 ZeroBot 中枢调度层跑通，实现基础的 LLM 对话、意图路由，以及简单的工具调用。
**预计耗时：** 3-4 周

* [x] **基础设施搭建** ☑️：
  * 部署 ZeroBot 网关（Golang/Rust），打通 飞书/钉钉 Webhook。☑️ *services/zero-gateway/ (Rust Axum)、services/zero-channels/ (10 个 IM 渠道)*
  * 搭建全局记忆层（Milvus 向量库 + Neo4j 图数据库）。☑️ *因果链图数据库 (Phase 16)、Markdown 记忆系统*
* [x] **中枢调度开发** ☑️：
  * 实现 LLM Router（根据 Prompt 路由到不同模型）。☑️ *packages/ccode/src/api/server/handlers/llm-router.ts*
  * 实现基础的安全数据脱敏（DLP Filter）。☑️ *packages/ccode/src/api/server/handlers/dlp.ts*
* [x] **标准工具集闭环** ☑️：
  * 开发简单的 API 工具（如天气、基础数据库查询），验证【AI -> Event Bus -> 执行 -> 返回】的全链路。☑️ *MCP 集成、30 个 Agent 实现、HTTP API 闭环*

---

### Phase 2: Web 深度解析与记忆演化 (硬核攻坚 - 网页端) 🔄 进行中

**目标：** 攻克你在待办中提到的”自动接口记忆”、”前端JS分析”等痛点，让系统具备**举一反三**的能力，不再每次重新推理。
**预计耗时：** 4-6 周

* [x] **动态接口拦截与嗅探 (Web Reverse Engine)** ☑️：
  * 集成 Playwright 拦截浏览器请求，自动提取未公开的 API 路径、Header 和 Token。☑️ *2026-02-18 Playwright 集成完成*
* [ ] **前端 JS 逆向与 AST 解析**：
  * 开发 JS 解析器，识别前端的数据加密/计算逻辑（如各类 Sign 签名算法），提取为 Python/Node 工具函数。*code-reverse agent 仅规划级别*
* [x] **认知与获取方式缓存 (Memory Cache)** ☑️：
  * 建立 `Schema 记忆库`。当 AI 成功跑通一次数据抓取（网页或接口），自动将其请求体结构和解析规则（XPath/JSONPath）作为”经验”存入缓存。☑️ *因果链图数据库 (Phase 16)*
  * **自纠错机制**：如果下次使用缓存报错（网页改版），触发 Sandbox 重新抓包分析。☑️ *REPL 闭环自纠错在 autonomous agent 中实现*
