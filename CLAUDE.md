...
项目定位是「既有自动档，也有手动档的AI代理观察系统」，应具备自主解决未知答案的复杂问题的能力。

1. 自动触发automous模式，在必要时code-coder能自主解决问题（产生的所有持久化的文件都应在 workspace下，因为ccode/zero-*全部视为二进制，不能修改）：
2. 能自主生成skill、prompt等纯markdown格式的内容，
3. 自主搜索web信息，自主逆向并提取/持久化api，提高效率
4. 自主搜寻github上可以解决问题的库/软件/程序并使用docker/wasm/虚拟环境的方式运行，
5. 自主生成tool脚本工具
6. 当前工具claude code只能是触发和检查的角色，不能参与实际解决该问题的过程，claude code只去触发code-coder，其他啥也不干
