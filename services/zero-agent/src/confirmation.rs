...
impl ConfirmationResponse {
    ...
    /// Check if this is an "always approve" response.
    ...
    match tokio::time::timeout(timeout, rx).await {
        Ok(Ok(response)) => {
            let result_msg = match response {
                ConfirmationResponse::Once => "✅ Approved",
                ConfirmationResponse::Always => "✅ Always approved",
                ConfirmationResponse::Reject => "❌ Rejected",
            };
            // Update the message to show result
            let _ = sink
                .update_confirmation_result(channel, user_id, response.is_approved(), result_msg)
                .await;
            Ok(response)
        }
        Ok(Err(_)) => {
        ...
        Err(_) => {
            ...
            let _ = sink
                .update_confirmation_result(channel, user_id, false, "⏱️ Timed out")
                .await;
            Err(anyhow::anyhow!(
                "Confirmation request timed out after {} seconds",
                timeout.as_secs()
            ))
        }
    }
...
pub async fn handle_confirmation_response_with_type(
    request_id: &str,
...
impl ConfirmationRegistry {
    ...
    /// List all pending confirmation IDs.
...
mod tests {
    ...
    async fn confirmation_registry_respond_reject() {
        ...
        registry
            .respond("req-2", ConfirmationResponse::Reject)
            .await;
    ...
    async fn confirmation_registry_respond_unknown() {
        ...
        let responded = registry
            .respond("unknown-req", ConfirmationResponse::Once)
            .await;
        assert!(!responded);
    }
    ...
    async fn confirmation_registry_multiple_concurrent() {
        ...
        let (r1, r2) = tokio::join!(
            async move {
                reg1.respond("concurrent-1", ConfirmationResponse::Once)
                    .await
            },
            async move {
                reg2.respond("concurrent-2", ConfirmationResponse::Reject)
                    .await
            }
        );
    ...
    #[test]
    fn confirmation_response_methods() {
        assert!(ConfirmationResponse::Once.is_approved());
        assert!(ConfirmationResponse::Always.is_approved());
        assert!(!ConfirmationResponse::Reject.is_approved());

        assert!(!ConfirmationResponse::Once.is_always());
        assert!(ConfirmationResponse::Always.is_always());
        assert!(!ConfirmationResponse::Reject.is_always());
    }
}
