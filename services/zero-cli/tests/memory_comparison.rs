...
use zero_cli::memory::{MarkdownMemory, Memory, MemoryCategory, SqliteMemory};
...
fn markdown_backend(dir: &std::path::Path) -> MarkdownMemory {
    MarkdownMemory::new(dir).expect("Markdown init failed")
}
...
async fn compare_store_speed() {
    ...
    // Both should succeed
    assert_eq!(sq.count(None).await.unwrap(), n);
    // Markdown count parses lines, may differ slightly from n
    let md_count = md.count(None).await.unwrap();
...
async fn compare_recall_quality() {
    ...
    let entries = vec![
        ...
        (
            "proj1",
            "Working on ZeroBot AI assistant",
...
async fn compare_upsert() {
    ...
    let sq_count = sq.count(None).await.unwrap();
    let md_count = md.count(None).await.unwrap();
...
async fn compare_forget() {
    ...
    // SQLite can delete
    assert!(sq_forgot);
    assert_eq!(sq.count(None).await.unwrap(), 0);
