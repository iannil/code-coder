...
// in `~/.codecoder/.secret_key` with restrictive file permissions (0600). The
...
/// Used by tests to verify key generation.
#[cfg(test)]
...
pub struct SecretStore {
    /// Path to the key file (`~/.codecoder/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(zerobot_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: zerobot_dir.join(".secret_key"),
            enabled,
        }
    }
