...
pub fn run_wizard() -> Result<Config> {
    ...
    println!(
        "  {}",
        style("An observation-centric AI agent system — observe first, control always.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 8, "Channels (How You Talk to ZeroBot)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("ZERO_BOT_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to ZeroBot)");
...
/// Use `zero-bot onboard` or `zero-bot onboard --api-key sk-... --provider openrouter --memory sqlite`.
/// Use `zero-bot onboard --interactive` for the full wizard.
...
pub fn run_quick_setup(
    ...
    if api_key.is_none() {
        ...
        println!("    1. Chat:     zero-bot agent -m \"Hello!\"");
        println!("    2. Gateway:  zero-bot gateway");
        println!("    3. Status:   zero-bot status");
    }
    println!();
...
fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".codecoder");
    ...
    let zerobot_dir = if use_default {
        default_dir
    } else {
...
fn setup_provider() -> Result<(String, String, String)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("ZeroBot works with ANY API that speaks the OpenAI chat completions format.");
        ...
        || provider_name == "google-gemini"
    {
        ...
        if zero_hub::gateway::GeminiProvider::has_any_auth() {
            ...
            print_bullet("ZeroBot will reuse your existing Gemini CLI authentication.");
            println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to ZeroBot from anywhere.");
        ...
        match choice {
            0 => {
                ...
                println!(
                    ...
                    style("— talk to ZeroBot from Telegram").dim()
                );
            ...
            1 => {
                ...
                println!(
                    ...
                    style("— talk to ZeroBot from Discord").dim()
                );
            ...
            2 => {
                ...
                println!(
                    ...
                    style("— talk to ZeroBot from Slack").dim()
                );
            ...
            3 => {
                ...
                print_bullet("ZeroBot reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            5 => {
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("zero-bot-whatsapp-verify".into())
                    .interact_text()?;
                ...
                config.whatsapp = Some(WhatsAppConfig {
                    ...
                    allowed_numbers,
                    app_secret: None, // Can be set via ZERO_BOT_WHATSAPP_APP_SECRET env var
                });
            }
            6 => {
                // ── Webhook ──
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("ZeroBot is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if has_channels {
        ...
        println!("       {}", style("zero-bot channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("zero-bot agent -m \"Hello, ZeroBot!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("zero-bot agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("zero-bot status").yellow());
...
mod tests {
    ...
    fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(
            identity.contains("**Name:** ZeroBot"),
            "should default agent name to ZeroBot"
        );
    ...
    fn scaffold_handles_special_characters_in_names() {
        ...
        let soul = fs::read_to_string(tmp.path().join("SOUL.md")).unwrap();
        assert!(soul.contains("ZeroBot-v2"));
    }
