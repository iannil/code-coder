...
use super::economic_bridge::{EconomicDataBridge, IndicatorConfig, IndicatorType, DataSource};
use super::hands::{HandsScheduler, HandExecution, HandSummary};
use super::monitor_bridge::{MonitorBridge, MonitorReport, MonitorRunResult};
    ...
    let summaries: Vec<WorkflowSummary> = workflows
        ...
        .map(|w| WorkflowSummary {
            ...
            trigger_type: match &w.trigger {
                super::workflow::Trigger::Webhook { .. } => "webhook".into(),
                super::workflow::Trigger::Cron { .. } => "cron".into(),
                super::workflow::Trigger::Manual => "manual".into(),
            },
...
async fn get_hand(
    State(state): State<Arc<WorkflowState>>,
    Path(id): Path<String>,
) -> Result<Json<ApiResponse<super::hands::HandManifest>>, StatusCode> {
    let hands_scheduler = state.hands_scheduler.read().await;
...
mod tests {
    ...
    fn create_test_state() -> Arc<WorkflowState> {
        ...
        let tmp = tempfile::TempDir::new().expect("Failed to create temp dir");
        let scheduler = crate::workflow::scheduler::Scheduler::with_data_dir(tmp.path().to_path_buf());
