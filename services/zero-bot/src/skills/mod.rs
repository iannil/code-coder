...
/// Skills live in `~/.codecoder/workspace/skills/<name>/SKILL.md`
...
fn open_skills_enabled() -> bool {
    if let Ok(raw) = std::env::var("ZERO_BOT_OPEN_SKILLS_ENABLED") {
...
fn resolve_open_skills_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("ZERO_BOT_OPEN_SKILLS_DIR") {
    ...
    if !readme.exists() {
        std::fs::write(
            ...
            "# ZeroBot Skills\n\n\
             ...
             ```bash\n\
             zero-bot skills install <github-url>\n\
             zero-bot skills list\n\
             ```\n",
        )?;
    }
    ...
    match command {
        crate::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.codecoder/workspace/skills/my-skill");
                println!("              echo '# My Skill' > ~/.codecoder/workspace/skills/my-skill/SKILL.md");
                println!();
                println!("  Or install: zero-bot skills install <github-url>");
            } else {
        ...
        crate::SkillCommands::Install { source } => {
            ...
            if source.starts_with("https://") || source.starts_with("http://") {
                ...
                if output.status.success() {
                    ...
                    println!("  Restart `zero-bot channel start` to activate.");
                } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.codecoder");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.codecoder/skills"));
    }
