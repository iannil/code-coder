...
/// `ZeroBot` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "zero-bot")]
...
enum MigrateCommands {
    /// Import memory from an `OpenClaw` workspace into this `ZeroBot` workspace
    Openclaw {
...
async fn main() -> Result<()> {
    ...
    } = &cli.command
    {
        ...
        if std::env::var("ZERO_BOT_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            if port == 0 {
                info!("🚀 Starting ZeroBot Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting ZeroBot Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            if port == 0 {
                info!("🧠 Starting ZeroBot Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting ZeroBot Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🐦‍🔥 ZeroBot Status");
            println!();
