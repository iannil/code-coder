...
pub struct Config {
    ...
    #[serde(default)]
    pub audit: super::super::audit::AuditConfig,
...
impl Config {
    ...
    pub fn load() -> Result<Self> {
        ...
        let merged = super::super::config_loader::load_modular_config(None)?;
    ...
    /// Get the unified daemon port (all services on single port).
    ...
    /// Get the Gateway service endpoint URL (unified daemon with path prefix).
    ///
    /// Uses the unified daemon port (4402) with /gateway path prefix.
    /// Example: "http://127.0.0.1:4402/gateway"
    pub fn gateway_endpoint(&self) -> String {
        format!("http://{}:{}/gateway", self.bind_address(), self.daemon_port())
    }

    /// Get the Channels service endpoint URL (unified daemon with path prefix).
    ///
    /// Uses the unified daemon port (4402) with /channels path prefix.
    /// Example: "http://127.0.0.1:4402/channels"
    pub fn channels_endpoint(&self) -> String {
        format!("http://{}:{}/channels", self.bind_address(), self.daemon_port())
    }

    /// Get the Workflow service endpoint URL (unified daemon with path prefix).
    ///
    /// Uses the unified daemon port (4402) with /workflow path prefix.
    /// Example: "http://127.0.0.1:4402/workflow"
    pub fn workflow_endpoint(&self) -> String {
        format!("http://{}:{}/workflow", self.bind_address(), self.daemon_port())
    }
